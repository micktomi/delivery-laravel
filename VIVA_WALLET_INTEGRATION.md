...
- Το `viva:reconcile-pending-payments` **συνεχίζει** να τρέχει, ώστε και ένα webhook που *χάθηκε* όσο ο διακόπτης ήταν κλειστός να ανακτάται αυτόματα. Εξετάζει μόνο παραγγελίες που έχουν ήδη φτάσει στο Smart Checkout (έχουν `viva_order_code`) και καμία διαδρομή του δεν μπορεί να δημιουργήσει payment order, οπότε δεν παρακάμπτει τον διακόπτη. Αν λείπουν και τα Merchant API credentials, η εντολή βγαίνει σιωπηλά με επιτυχία: μια αποσυναρμολογημένη ενσωμάτωση δεν πρέπει να αποτυγχάνει κάθε 5 λεπτά στο cron.

Με μια πρόταση: **ο διακόπτης σταματά μόνο νέες πληρωμές· ό,τι έχει ήδη κινηθεί συνεχίζει να προσγειώνεται.**
...
Χρειάζονται όσο υπάρχουν Viva παραγγελίες σε εξέλιξη — και με `VIVA_ENABLED=false`, γιατί από αυτά εξαρτάται η ανάκτηση μιας πληρωμής που κινήθηκε πριν κλείσει ο διακόπτης. Αν λείπουν ενώ `VIVA_ENABLED=true`, η εντολή reconciliation τερματίζει με failure και γράφει ασφαλές operational event χωρίς να αλλάξει παραγγελίες· αν λείπουν ενώ η Viva είναι ήδη απενεργοποιημένη, βγαίνει σιωπηλά με επιτυχία. Και στις δύο περιπτώσεις η ακύρωση μιας pending Viva παραγγελίας από τον admin απορρίπτεται, ώστε να μην ακυρωθεί τοπικά κάτι που παραμένει πληρωτέο στη Viva.
